...
claude plugin install fin-winner-plugin@xcan
```
...
codex plugin add fin-winner-plugin@xcan
```
...
```text
.claude-plugin/marketplace.json   # Claude Code marketplace（名称：xcan）
.agents/plugins/marketplace.json  # Codex / ChatGPT 桌面端 marketplace（名称：xcan）
plugins/<plugin>/
...
- 任何插件包内容变更（清单、`.mcp.json`、skills）都必须同步升级版本号：
  ...
  缓存已安装的插件副本。
- 安装坐标（`<plugin>@xcan`）是面向用户的稳定接口，插件名和 marketplace
  名一经发布不可更名。
